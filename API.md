...
API 摘要，覆盖主要公开类与要点。  
API summary covering main public classes and key points.
...
方法 / Methods:
- `void Validate()` — 对设置进行防护性校验与 clamp（在扫描前会被自动调用）。Validates and clamps settings (auto-called before scan).

### RDF_LidarSample
单条射线的采样结果数据结构。字段 / Single-ray sample result. Fields:
- `m_Index` (int): 射线索引
- `m_Hit` (bool): 是否命中
- `m_Start` (vector): 射线起点
...
备注 / Note: 默认采样策略为 `RDF_UniformSampleStrategy` / Default sample strategy is `RDF_UniformSampleStrategy`.
...
颜色策略 / Color strategy: 可用 `RDF_LidarColorStrategy` 替换默认色彩映射 / Replace default color mapping. 需自定义导出时 / For custom export: 使用 `GetLastSamples()` 取得数据后自行处理（如 CSV/JSON）/ use `GetLastSamples()` then process (e.g. CSV/JSON).

---

## Strategies

### RDF_LidarSampleStrategy (接口 / Interface)
- `vector BuildDirection(int index, int count)` — 返回单位方向向量 / Returns unit direction vector.

实现 / Implementations: `RDF_UniformSampleStrategy`、`RDF_HemisphereSampleStrategy`、`RDF_ConicalSampleStrategy`、`RDF_StratifiedSampleStrategy`、`RDF_ScanlineSampleStrategy`、`RDF_SweepSampleStrategy`、`RDF_RectangularFOVSampleStrategy`（矩形视场，Example 项目 / rectangular FOV, Example project）。
...
### RDF_LidarColorStrategy (接口 / Interface)
...
实现 / Implementations: `RDF_DefaultColorStrategy`、`RDF_ThreeColorStrategy`（近/中/远三段渐变，默认绿→黄→红 / near/mid/far gradient, default green→yellow→red）。
...
用于解析扫描主体（本地玩家或当前载具）的静态工具类。Static utility for resolving scan subject (local player or current vehicle).
...
### RDF_LidarExport
将扫描结果导出为 CSV 格式（控制台或磁盘）。Export scan results to CSV (console or disk).
...
- `static bool ExportToFile(array<ref RDF_LidarSample> samples, string path)` — 写入 CSV 文件（覆盖）
- `static bool AppendToFile(array<ref RDF_LidarSample> samples, string path, bool writeHeaderIfNew)` — 追加到 CSV 文件
- `static string GetExtendedCSVHeader()` — 扩展表头（含 time、origin、elevation、azimuth、subjectVel 等）
- `static string SampleToExtendedCSVRow(sample, currentTime, maxRange, subjectVel, subjectYaw, subjectPitch, scanId, frameIndex)` — 扩展 CSV 行

### RDF_LidarSampleUtils
对样本数组的统计与过滤（静态方法）。Statistics and filtering for sample arrays (static methods).
...
### RDF_LidarNetworkAPI
网络同步 API 基类（默认空实现），用于解耦 Demo 与网络逻辑。Network sync API base class (empty default), decouples Demo from network logic.
...
Network 模块内置实现，基于 Rpl 同步状态与扫描结果。Built-in Network implementation, syncs state and scan results via Rpl.

同步属性（原子字段）/ Sync props (atomic fields):
...
备注 / Note: 配置通过 RPC 以原子字段形式传输 / Config transmitted via RPC as atomic fields.

扫描结果 / Scan results: 服务器通过 `RDF_LidarExport.SamplesToCSV()` 将样本序列化为紧凑 CSV 字符串，并通过不可靠 RPC `RpcDo_ScanCompleteWithPayload(string csv)` 广播到客户端；客户端使用 `RDF_LidarExport.ParseCSVToSamples(csv)` 解析回 `RDF_LidarSample` 列表。Server serializes via `SamplesToCSV()`, broadcasts via RPC; client parses with `ParseCSVToSamples()`.

### RDF_LidarAutoRunner 网络集成 / Network integration
- `static void SetNetworkAPI(RDF_LidarNetworkAPI networkAPI)` — 设置网络 API 引用
- `static RDF_LidarNetworkAPI GetNetworkAPI()` — 获取网络 API

当设置网络 API 后，AutoRunner 会使用服务器权威的扫描与同步状态变化。When set, AutoRunner uses server-authoritative scan and sync.

### RDF_LidarNetworkUtils
网络辅助工具 / Network utilities:
...
网络扫描适配器（非 Demo）/ Network scan adapter (non-Demo):
...
### RDF_LidarExport 网络序列化 / Network serialization
...
### RDF_LidarVisualizer 网络支持 / Network support
- `void RenderWithSamples(IEntity subject, array<ref RDF_LidarSample> samples)` — 使用预计算样本渲染（用于网络同步）

## Demo（统一 API / Unified API）

### 统一开关 / Unified switch
- **RDF_LidarAutoRunner** 为演示的唯一起动入口，所有演示通过 **SetDemoEnabled** 与 **SetDemoConfig / StartWithConfig** 控制。RDF_LidarAutoRunner is the sole demo entry; all demos controlled via SetDemoEnabled and SetDemoConfig/StartWithConfig.
...
### RDF_LidarDemoConfig
配置对象，**推荐通过静态预设工厂创建**，再配合 `SetDemoConfig` / `StartWithConfig` 使用。Config object; **recommended: create via static preset factories**, then use with SetDemoConfig/StartWithConfig:
...
**预设工厂（替代原 RDF_*Demo.Start）/ Preset factories (replaces RDF_*Demo.Start):**
...
方法 / Methods:
- `void ApplyTo(RDF_LidarAutoRunner runner)` — 将配置应用到 runner / Apply config to runner（内部通过 AutoRunner 的 SetDemo* API 实现 / via AutoRunner SetDemo* API）.

### RDF_LidarDemoCycler
策略轮换，仅调用 `RDF_LidarAutoRunner.SetDemoConfig` + `SetDemoEnabled`。Strategy cycler; only calls SetDemoConfig + SetDemoEnabled:
...
### SCR_BaseGameMode（统一 Bootstrap / Unified Bootstrap）
...
## 仅逻辑不渲染（无可视化）/ Logic-only (no visualization)

若只需要扫描数据做逻辑（威胁检测、距离判断等），**不必使用 RDF_LidarVisualizer**，直接使用 Scanner 即可，避免创建 debug shapes。  
If you only need scan data for logic (threat detection, range checks, etc.), **do not use RDF_LidarVisualizer**; use Scanner directly to avoid creating debug shapes:
...
For AutoRunner with post-scan handling, set `SetScanCompleteHandler` and use `RDF_LidarSampleUtils` or `RDF_LidarExport.PrintCSVToConsole(samples)` in the callback.

---

使用示例和注意事项在 `README.md` 中。Usage examples and notes in `README.md`.