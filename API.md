...
示例：
```c
// 在高射线计数场景启用批量三角网格渲染以减少 Shape 调用
RDF_LidarAutoRunner.SetDemoUseBatchedMesh(true);
RDF_LidarAutoRunner.SetDemoConfig(RDF_LidarDemoConfig.CreateDefault(1024));
RDF_LidarAutoRunner.SetDemoEnabled(true);

// 查询当前状态
bool usingBatched = RDF_LidarAutoRunner.GetDemoUseBatchedMesh();
Print("RDF: Using batched mesh = " + usingBatched);
```
